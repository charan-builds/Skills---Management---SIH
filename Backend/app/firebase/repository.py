...
class FirestoreRepository:
    ...
    def add_trainee_employment(trainee_id: str, emp: TraineeEmploymentCreate) -> Optional[Dict[str, Any]]:
        ...
        # Update current outcome status on the trainee profile
    ...
    def add_trainee_followup(trainee_id: str, followup: TraineeFollowupSubmit) -> Optional[Dict[str, Any]]:
        ...
        # Update existing checkpoint or add new one
    ...
    def get_intervention(intervention_id: str) -> Optional[Dict[str, Any]]:
        from app.core.config import settings
        if settings.ENABLE_DEMO_MODE:
            interventions = FirestoreRepository._load_local_demo_data().get('interventions', [])
            for inv in interventions:
                if inv.get("id") == intervention_id:
                    return inv
            return None
    ...
    @staticmethod
    def create_intervention(intervention_data: Dict[str, Any]) -> Dict[str, Any]:
        i_id = intervention_data.get("id") or f"INV{uuid.uuid4().hex[:4].upper()}"
        intervention_data["id"] = i_id
        now = datetime.utcnow().isoformat() + "Z"
        intervention_data["created_at"] = now
        intervention_data["updated_at"] = now
        db.collection("interventions").document(i_id).set(intervention_data)
