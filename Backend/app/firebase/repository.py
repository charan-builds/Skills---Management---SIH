...
class FirestoreRepository:
    ...
    @classmethod
    def _save_local_demo_data(cls):
        """Persist demo data cache back to demo_data.json on disk so changes survive restarts."""
        if cls._demo_data_cache is None:
            return
        import json
        import os
        from app.core.config import BASE_DIR
        ...
        try:
            with open(demo_file, 'w', encoding='utf-8') as f:
                json.dump(cls._demo_data_cache, f, indent=2, ensure_ascii=False)
        except Exception as e:
            print(f'Failed to save demo data JSON: {e}')
    ...
    def create_programme(programme: ProgrammeCreate) -> Dict[str, Any]:
        ...
        if settings.ENABLE_DEMO_MODE:
            ...
            FirestoreRepository._save_local_demo_data()
        elif db:
            try:
        ...
        if settings.ENABLE_DEMO_MODE:
            ...
            for t in demo_data.get("trainees", []):
                if t.get("id") == trainee_id:
                    # Sync nested portal_state into top-level properties if present
                    if "portal_state" in update_data and isinstance(update_data["portal_state"], dict):
                        p_state = update_data["portal_state"]
                        p_info = p_state.get("personal_info") or {}
                        if p_info.get("name"): t["name"] = p_info["name"]
                        if p_info.get("email"): t["email"] = p_info["email"]
                        if p_info.get("location"): t["district"] = p_info["location"].split(",")[0].strip()
                        if p_state.get("skills"):
                            # Format skills array
                            raw_skills = p_state["skills"]
                            formatted_skills = [
                                s.get("name") if isinstance(s, dict) else str(s)
                                for s in raw_skills
                            ]
                            t["skills"] = formatted_skills
            ...
            try:
                ...
                if doc.exists:
                    if "portal_state" in update_data and isinstance(update_data["portal_state"], dict):
                        p_state = update_data["portal_state"]
                        p_info = p_state.get("personal_info") or {}
                        if p_info.get("name"): update_data["name"] = p_info["name"]
                        if p_info.get("email"): update_data["email"] = p_info["email"]
                        if p_info.get("location"): update_data["district"] = p_info["location"].split(",")[0].strip()
                        if p_state.get("skills"):
                            raw_skills = p_state["skills"]
                            update_data["skills"] = [
                                s.get("name") if isinstance(s, dict) else str(s)
    ...
    def get_job_applications(
        ...
        from app.core.config import settings
        if settings.ENABLE_DEMO_MODE:
            apps = FirestoreRepository._load_local_demo_data().get("job_applications", [])
            if job_id:
                apps = [a for a in apps if a.get("job_id") == job_id]
            if trainee_id:
                apps = [a for a in apps if a.get("trainee_id") == trainee_id]
            if employer_id:
                apps = [a for a in apps if a.get("employer_id") == employer_id]
            return apps
        if db:
            query = db.collection("job_applications")
            if job_id:
    ...
    @staticmethod
    def get_job_application(application_id: str) -> Optional[Dict[str, Any]]:
        from app.core.config import settings
        if settings.ENABLE_DEMO_MODE:
            apps = FirestoreRepository._load_local_demo_data().get("job_applications", [])
            return next((a for a in apps if a.get("id") == application_id), None)
        if db:
            doc = db.collection("job_applications").document(application_id).get()
            data = doc.to_dict() if doc.exists else None
            return data if FirestoreRepository._should_include(data) else None
        return None

    @staticmethod
    def create_job_application(app_data: Dict[str, Any]) -> Dict[str, Any]:
        from app.core.config import settings
        ...
        app_data.setdefault("status", "Applied")
        if settings.ENABLE_DEMO_MODE:
            demo_data = FirestoreRepository._load_local_demo_data()
            demo_data.setdefault("job_applications", []).append(app_data)
            FirestoreRepository._save_local_demo_data()
        elif db:
            try:
                db.collection("job_applications").document(app_id).set(app_data)
            except Exception as e:
                print(f"Firestore write error in create_job_application: {e}")
                raise
        return app_data

    @staticmethod
    def update_job_application_status(application_id: str, status: str, employer_notes: Optional[str] = None) -> Optional[Dict[str, Any]]:
        from app.core.config import settings
        now = datetime.utcnow().isoformat() + "Z"
        updates = {"status": status, "updated_at": now}
        if employer_notes is not None:
            updates["employer_notes"] = employer_notes

        updated_app = None
        if settings.ENABLE_DEMO_MODE:
            demo_data = FirestoreRepository._load_local_demo_data()
            for a in demo_data.get("job_applications", []):
            ...
            try:
                ...
                if doc.exists:
                    ...
                    updated_app = doc_ref.get().to_dict()
            except Exception as e:
                print(f"Firestore update error in update_job_application_status: {e}")
                raise
        return updated_app

    # --- EMAIL OTP REPOSITORY METHODS ---

    @staticmethod
    def save_otp_record(otp_record: Dict[str, Any]) -> Dict[str, Any]:
        from app.core.config import settings
        if settings.ENABLE_DEMO_MODE:
            demo_data = FirestoreRepository._load_local_demo_data()
            demo_data.setdefault("email_otps", []).append(otp_record)
            FirestoreRepository._save_local_demo_data()
        elif db:
            try:
                db.collection("email_otps").document(otp_record["id"]).set(otp_record)
            except Exception as e:
                print(f"Firestore write error in save_otp_record: {e}")
                raise
        return otp_record

    @staticmethod
    def get_latest_otp_record(email: str) -> Optional[Dict[str, Any]]:
        from app.core.config import settings
        email_clean = email.strip().lower()
        if settings.ENABLE_DEMO_MODE:
            demo_data = FirestoreRepository._load_local_demo_data()
            otps = [
                o for o in demo_data.get("email_otps", [])
                if o.get("email", "").strip().lower() == email_clean
            ]
            if not otps:
                return None
            otps.sort(key=lambda x: x.get("createdAt", ""), reverse=True)
            return otps[0]
        elif db:
            try:
                docs = (
                ...
                print(f"Firestore query error in get_latest_otp_record: {e}")
                return None
        return None

    @staticmethod
    def update_otp_record(otp_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        from app.core.config import settings
        if settings.ENABLE_DEMO_MODE:
            demo_data = FirestoreRepository._load_local_demo_data()
            for o in demo_data.get("email_otps", []):
                if o.get("id") == otp_id:
                    o.update(updates)
                    FirestoreRepository._save_local_demo_data()
                    return o
            ...
            try:
                doc_ref = db.collection("email_otps").document(otp_id)
                doc_ref.update(updates)
                return doc_ref.get().to_dict()
            except Exception as e:
                print(f"Firestore update error in update_otp_record: {e}")
                return None
        return None

    @staticmethod
    def revoke_active_otps_for_email(email: str) -> None:
        from app.core.config import settings
        email_clean = email.strip().lower()
        if settings.ENABLE_DEMO_MODE:
            demo_data = FirestoreRepository._load_local_demo_data()
            changed = False
            for o in demo_data.get("email_otps", []):
                if o.get("email", "").strip().lower() == email_clean and o.get("status") == "ACTIVE":
                    o["status"] = "REVOKED"
                    changed = True
            if changed:
                FirestoreRepository._save_local_demo_data()
            ...
            try:
                ...
                for d in docs:
                    d.reference.update({"status": "REVOKED"})
            except Exception as e:
                print(f"Firestore update error in revoke_active_otps_for_email: {e}")
