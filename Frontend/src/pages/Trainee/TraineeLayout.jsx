...
export default function TraineeLayout({ activeTab, onTabChange, children, portalData }) {
  ...
  const navItems = [
    ...
    { id: 'jobs', label: 'Explore Jobs', icon: Briefcase, route: `/trainee/jobs` },
  ...
  const userName = portalData?.personal_info?.name || localStorage.getItem("traineeName") || "Trainee";
  const userInitials = userName.split(" ").filter(Boolean).map(n => n[0]).join("").toUpperCase() || "TR";
